...
function getFuelIcon(type, size = 14) {
...
Object.assign(window.appUiMixin, {
    ...
    updateSearchFormUI() {
        ...
        if (cityInput)
            cityInput.value = this.formData.city;
        if (radiusInput)
        ...
        resultsSlider?.setAttribute("aria-valuetext", `${this.formData.results} ${this.translate("results_heading", "Results")}`);
        if (radiusLabel)
            radiusLabel.textContent = this.translate("search_radius", "Raggio di Ricerca");
        if (resultsLabel)
            resultsLabel.textContent = this.translate("max_results", "Limite Risultati");
        if (submitBtn)
    ...
    renderResults() {
        ...
        for (const [index, station] of this.results.entries()) {
            ...
            const stationName = station.gestore || this.translate("station", "Gas Station");
