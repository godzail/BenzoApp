...
Object.assign(window.appUiMixin, {
    ...
    handleCityKeydown(e) {
        ...
        items.forEach((item, idx) => { if (item.classList.contains("bg-[var(--bg-elevated)]"))
    ...
    updateSearchButtonUI() {
        ...
        if (this.loading) {
            ...
            if (loadingText)
                loadingText.textContent = window.t ? window.t("loading", "Caricamento...") : "Caricamento...";
        }
        else {
    ...
    isCheapestStation(index) {
        if (!this.results || this.results.length === 0)
            return false;
        const prices = this.results.map((s) => Number(s.fuel_prices?.[0]?.price)).filter((p) => Number.isFinite(p));
        if (prices.length === 0)
            return false;
    ...
    async submitForm() {
        ...
        try {
            ...
            if (data.warning) {
                ...
                this.results = data.stations?.map((station) => ({ ...station, gestore: window.extractGestore(station), distance: station.distance || "" })) || [];
                this.error = "";
            }
            this.updateResultsUI?.();
            Promise.resolve().then(() => { this.updateMap?.(); if (window.updateI18nTexts)
                window.updateI18nTexts(); });
        }
        catch (err) {
    ...
    placeMapAccordingToViewport() {
        try {
            ...
            setTimeout(() => { this.map?.invalidateSize?.(); this.initMap?.(); }, 100);
        }
        catch (err) {
