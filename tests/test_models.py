...
def test_stationsearchparams_lat_lon_bounds():
...
def test_stationsearchparams_distance_and_results_bounds_and_defaults():
    with pytest.raises(ValidationError):
...
def test_defaults_for_stationsearchparams_and_searchrequest_and_models():
...
def test_fuelprice_non_negative():
    with pytest.raises(ValidationError):
...
def test_station_coords_and_distance_validation():
...
def test_settings_cors_allowed_origins_trim(monkeypatch):
